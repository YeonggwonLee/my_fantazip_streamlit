...
그 중에서도 특히 2030 사회초년생 여성들에게 **살기 좋은 동네 = 안전한 동네**일 것이라고 생각을 했습니다.\
그래서 저희가 모은 데이터로 저희 프로젝트만의 안전과 관련된 기준점을 찾고,\
범죄로부터 안전한 동네에 대한 정보를 찾고 제공하고자 했습니다.")
st.write("목표 = 2030여성 사회초년생 1인가구에게 안전한 동네 찾기")
# 한계 및 대안
st.info("**<한계 및 대안>**")
st.write(" 안전한 동네인지 판단하기 위해 범죄 데이터를 보고 판단하려고 했지만 동 단위의 범죄발생 데이터는 구하지 못했다는 한계가 있었습니다. \
    그래서 저희가 생각한 대안은 자치구별 범죄 데이터는 있으니 자치구별 범죄 발생과 연관이 있는 다른 요소들을 찾고 이를 동단위에 적용해서 보고자 했습니다.")
st.write(" 따라서 해당 페이지에서는")
st.markdown(
        """
        1. 자치구별로 어던 곳이 위험하고 안전한지 살펴보기.
        2. 범죄 발생과 어떤 요소가 연관이 있는지 살펴보기.
        """
)
st.write("이 2가지에 대해 중점적으로 보도록 하겠습니다.")
# 핵심 가설
st.info("**<핵심 가설>**")
st.write("1. CCTV가 많으면 범죄발생이 적을 것이다. ➡️ **🤍안전 vs 위험지역 특징 찾기 - 거주환경**에서 확인하실 수 있습니다.")
st.write("2. 가로등이 많으면 범죄발생이 적을 것이다. ➡️ **🤍안전 vs 위험지역 특징 찾기 - 거주환경**에서 확인하실 수 있습니다.")
st.write("3. 주변에 유흥주점 수가 적으면 안전할 것이다. ➡️ **🤍안전 vs 위험지역 특징 찾기 - 생활환경**에서 확인하실 수 있습니다.")
st.write("4. 2030여성 1인가구 거주 비율이 높으면 안전할 것이다. ➡️ **🤍안전 vs 위험지역 특징 찾기 - 생활환경**에서 확인하실 수 있습니다.")

st.warning("해당 프로젝트는 서울을 기준으로 하고 있습니다.")
...
if rank:
    st.write("🕵️‍♀️: 자치구별로 범죄발생 건수와 2030여성인구 1000명대비 범죄발생 건수를 순위로(많이 발생하면 1등, 적게 발생하면 25등) 나타낸 뒤 평균을 내서 오름차순으로 정리한 결과를 사용하였습니다.")
    st.image("./data/gu_rank.png")
    # st.write("**☄️강남구, 서초구, 영등포구**")
    # st.write("**🟢도봉구, 성동구, 성북구**")

# st.subheader("< 결과 >")
st.info("위의 🧐**순위 알아보기**에서 자치구별로 범죄발생 건수와 2030여성인구 1000명당 범죄발생 건수를 순위로(많이 발생하면 1등, 적게 발생하면 25등) 나타낸 뒤 \
    순위에 대한 평균을 계산한 결과를 토대로 아래와 같은 결과를 얻을 수 있었습니다.")
st.write("**위험🔴 : 강남구, 서초구, 영등포구** / **안전🟢 : 도봉구, 성동구, 성북구**")
st.write("\n")
# st.write("➡️ 부자 동네가 위험지역이다?!")
st.warning("다음 내용")
st.write("위험지역에 속한 곳은 소위 말해 부자동네 또는 업무중심지구라는 특징을 가지고 있습니다.\
    하지만 안전지역은 이렇다할 특징이 떠오르지 않네요..\
        다음 단계에서 각각 어떤 특징을 가지고 있는지 살펴보도록 하겠습니다.")
...
# with st.expander("**2. 2030여성인구 대비 범죄발생 살펴보기**"):
    ...
    if crime_ttest:
        ...
        st.info("결론: 2030여성 기준 위험지역에서 평균 범죄발생 건수가 안전지역보다 통계적으로 많이 발생한다.")
...
st.write("\n")
st.info("**🤍안전 vs 위험지역 특징 찾기** 요약")
st.markdown(
    """
    - 범죄발생 건수, 2030여성인구 비율 모두 위험지역에 속한 곳들에서 범죄가 많이 발생한다.\
        2030여성인구에 따른 범죄발생을 평균으로 봐도 위험지역에서 높은 발생 건수를 보였다.
    - t-test 결과) 2030여성 기준, 위험지역에서 평균 범죄발생(일반범죄, 성범죄 모두) 건수가 안전지역보다 통계적으로 많이 발생한다.
    - 1인당 지역내 총생산을 확인할 결과, 위험지역에 속한 지역이 안전지역에 속한 곳보다 높았다.
    """
)
st.warning("다음 내용")
st.write("**🤍안전 vs 위험지역 특징 찾기**에서 위와 결론을 얻었습니다. 이외에 다른 특징은 무엇이 있을까요? 다른 특징을 더 알아보기 위해 '거주환경'과 '생활환경'으로 나눠서 특징을 살펴보도록 하겠습니다.")
st.markdown(
    """
    - 거주환경: CCTV, 가로등, 경찰서, 여성 안심지킴이집
    - 생활환경: 유흥주점, 전체 거주 인구, 2030여성 거주 인구, 1인가구(특히 2030여성)
    """
)
...
st.write("\n")
# 거주환경 요약
st.info("**🤍안전 vs 위험지역 특징 찾기 - 거주환경** 요약")
st.markdown(
    """
    - 안전지역의 설치 개수가 안전지역보다 낮다.
    - 안전지역의 평균 설치 개수가 위험지역에 비해 현저히 낮지만 경찰서 평균 개수는 비슷한 결과를 보인다.
    - 위험지역에 2030여성인구 대비 CCTV 수가 통계적으로 많이 설치되어 있었지만 다른 변수들은 통계적으로 위험지역과 안전지역 간의 차이가 없었다.
    """
)
# 다음 내용 예고
st.warning("다음 내용")
st.write("특징 찾기 - **거주환경** 부분에서 위와 같은 특징이 있다는 것을 확인했습니다. 다음은 **생활환경** 부분에 대한 특징을 알아보도록 하겠습니다.")
...
st.write("\n")
# 생활환경 요약
st.info("**🤍안전 vs 위험지역 특징 찾기 - 생활환경** 요약")
st.write()
st.markdown(
    """
    - 2030여성인구 대비 유흥주점 수가 위험지역에 통계적으로 많다.
    - 2030여성 1인가구 대비 유흥주점 수는 위험지역과 안전지역 간의 차이가 없었다.
    - 2030여성 1인가구 비율 평균은 위험지역이 조금 더 높았지만 통계적으로는 두 지역 간 차이는 없었다.
    """
)

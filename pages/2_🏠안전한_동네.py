...
with st.expander(label=f"🟡 **도선동**"): 
    ...
    df= pd.DataFrame({
                    ...
                    '70~100': '💜'}, index=['범례 색상'])
    st.dataframe(df)
    st.image("./data/도선동_인구연령.jpg", width=700)
...
with st.expander(label=f"**🔴 홍익동  VS 🟢 상왕십리동, 하왕십리동**"): 
    ...
    st.image("./data/성동구_인구연령.jpg", width=800)
    ...
    st.dataframe(df)
    st.image("./data/성동구_생활조건.jpg", width=700)
    df= pd.DataFrame({
                    'cctv': '💙',
...
with st.expander(label=f"**🔴 공항동  VS 🟢 등촌동, 염창동**"): 
    ...
    df= pd.DataFrame({
                    ...
                    '70~100': '💜'}, index=['범례 색상'])
    st.dataframe(df)
    st.image("./data/강서구_인구연령.jpg", width=800)
    ...
    df= pd.DataFrame({
                    ...
                    '여성안심지킴이집': '🩷'}, index=['위 그래프 범례 색상'])
    st.dataframe(df)
    st.image("./data/강서구_생활조건.jpg", width=700)
    df= pd.DataFrame({
                    'cctv': '💙',
